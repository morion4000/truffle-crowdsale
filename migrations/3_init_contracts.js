...
module.exports = function(deployer, network, accounts) {
	CapitalTechCrowdsale.deployed().then(inst => { 
		inst.token_call().then(addr => { CALLToken.at(addr).transferOwnership(inst.address)});
		inst.token_callg().then(addr => { CALLGToken.at(addr).transferOwnership(inst.address)});
		inst.powerUpContract();
	})
};